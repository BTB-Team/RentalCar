...
export const Navbar = () => {
  ...
  const handleFaqClick = (event) => {
    ...
    navigate("/");
    window.setTimeout(scrollToFaq, 0);
  };

  //    The link that is active will have yellow underline
  const linkClass = ({ isActive }) =>
    `font-extrabold text-[15px] pb-1 border-b-2 transition-colors ${
      isActive
  ...
  return (
    <>
      <header
        className={`fixed top-[15px] left-0 w-full z-50 h-[81px] transition-transform duration-500 ease-in-out ${
