...
export const Navbar = () => {
  ...
  const navLinks = [
    ...
    {
      to: "/",
      label: t.navbar.faq || (lang === "dr" ? "سوالات متداول" : "FAQ"),
      isFaq: true,
    },
  ];
  ...
  return (
    <>
      <header
        className={`fixed top-[15px] md:top-[39px] left-0 w-full z-50 h-auto transition-transform duration-500 ease-in-out ${isVisible ? "translate-y-0" : "-translate-y-[calc(100%+40px)]"
          }`}
      >
        <nav className="max-w-7xl w-full mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center bg-white sm:rounded-[20px] rounded-[10px] p-1.5 shadow-sm border border-gray-100 h-[48px] sm:h-[56px] gap-1">
            ...
            <button
              ...
              className="flex items-center justify-center px-3 sm:px-4 h-full w-[56px] sm:rounded-[16px] rounded-[8px] font-bold text-xs sm:text-sm text-brand-black transition-all duration-200 focus:outline-none"
            ...
            <button
              ...
              className="xl:hidden flex items-center justify-center p-2 sm:p-2.5 h-full sm:rounded-[16px] rounded-[8px] text-brand-black hover:bg-gray-100 transition-colors focus:outline-none"
            ...
            >
              {isMobileOpen ? (
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
                  className="w-5 h-5 sm:w-6 sm:h-6 text-brand-black"
                >
                  <path
