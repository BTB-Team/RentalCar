        ...
        >
          {navLinks.map((link) => {
            ...
            const getButtonClass = (isActive) =>
              `flex items-center justify-between px-5 py-3.5 rounded-2xl text-lg font-semibold transition-all duration-200 w-full ${
                isActive
              ...
              >
                {link.isFaq ? (
                  ...
                  <NavLink
                    to={link.to}
                    onClick={onClose}
                    className="w-full bg-brand-black flex items-center justify-center gap-2 text-white text-center py-4 rounded-xl font-bold text-[20px] shadow-md hover:bg-brand-yellow hover:text-brand-black transition-all duration-300"
                >
